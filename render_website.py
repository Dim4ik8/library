...
def on_reload(books_on_page, book_cards_in_row):
    for page, books in enumerate(books_on_page, start=1):
        books_in_column = list(chunked(books, book_cards_in_row))
...
def main():
    ...
    on_reload(books_on_page, book_cards_in_row)

    server = Server()
    server.watch('template.html', on_reload(books_on_page, book_cards_in_row))
    server.serve(root='.')
